...
@section('content')
  <div id="one-column" style="width: 80%">
    ...
    <form action="{{ route('materials.update', $material) }}" method="POST">
      ...
      <div class="input-group mb-4">
        {{-- <select name="category_key" class="form-select me-3">
          @foreach ($categories as $key => $category)
            <option value="{{ $key }}" @if($material->category_key == $key) selected @elseif(old('category_key') == $key) selected @endif>{{ $category }}</option>
          @endforeach
        </select> --}}

        <select name="type_key" class="form-select me-3">
          @foreach ($types as $key => $type)
            <option value="{{ $key }}" @if($material->type_key == $key) selected @elseif(old('type_key') == $key) selected @endif>{{ $type }}</option>
          @endforeach
        </select>
      ...
      <div class="input-group mb-4">
        <textarea name="description" rows="5" class="form-control">{{ old('description') ??$material->description }}</textarea>
      </div>
