      ...
      <form action="{{ route('search') }}" class="top-search__form">
        <div class="top-search__select">
          <select name="range" id="range">
            <option value="1" @if($range == 1) selected @endif>제목</option>
            <option value="2" @if($range == 2) selected @endif>전체</option>
          </select>
          <input type="search" name="word" id="word" value="{{ $word }}"/>
        </div>
        <div class="top-search__radio">
          <div>
            <input type="radio" name="category" id="radio-a" value="1" @if($category == 1) checked @endif />
            <label for="radio-a">전체</label>
          </div>
          <div>
            <input type="radio" name="category" id="radio-b" value="2" @if($category == 2) checked @endif/>
            <label for="radio-b">학습실</label>
          </div>
          <div>
            <input type="radio" name="category" id="radio-c" value="3" @if($category == 3) checked @endif/>
